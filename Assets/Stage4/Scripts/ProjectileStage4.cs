...
public class ProjectileStage4 : MonoBehaviour {
    [Header("Lifetime")]
    ...
    private void HandleHit(Collider other, Vector3 hitPoint) {
        ...
        TankDestructionSequence destructionSequence =
            other.GetComponentInParent<TankDestructionSequence>();

        if (destructionSequence != null) {
            destructionSequence.DestroyTank(hitPoint);
            Destroy(gameObject);
            return;
        }
